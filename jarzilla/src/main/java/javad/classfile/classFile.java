...
import javad.attr.attrFactory;
import javad.attr.attrInfo;
import javad.attr.codeAttr;
import javad.attr.constValueAttr;
import javad.attr.deprecAttr;
import javad.attr.exceptAttr;
import javad.attr.srcFileAttr;
import javad.attr.synthAttr;
import javad.jconst.constBase;
import javad.jconst.constClass_or_String;
import javad.jconst.constPool;
import javad.jconst.constUtf8;
import javad.util.accData;
import javad.util.accString;
import javad.util.access_and_modifier_flags;
import javad.util.dataRead;
import javad.util.errorMessage;
import javad.util.objNameFormat;
import javad.util.typeDesc;
...
class classFileHeader extends dataRead {
  ...
  classFileHeader( DataInputStream dStream ) {
    magic = this.readU4( dStream );
    minor_version = (short)this.readU2( dStream );
    major_version = (short)this.readU2( dStream );
...
class classDeclSec extends dataRead implements access_and_modifier_flags {
  ...
  classDeclSec( DataInputStream dStream, constPool constPoolSec ) {
    ...
    accessFlags = this.readU2( dStream );
    thisClassIx = this.readU2( dStream );
    superClassIx = this.readU2( dStream );

    thisClass = constPoolSec.constPoolElem( thisClassIx );
    superClass = constPoolSec.constPoolElem( superClassIx );

    interfaceCnt = this.readU2( dStream );
...
class fieldInfo extends dataRead {
  ...
  fieldInfo( DataInputStream dStream, constPool constPoolSec ) {
    ...
    access_flags = this.readU2( dStream );
    name_index   = this.readU2( dStream );
    desc_index   = this.readU2( dStream );
    attr_cnt     = this.readU2( dStream );
...
<p>
   ...
   */
  void pr() {
    System.out.println( this.fieldString() );
  } // pr
...
<p>
  ...
  String methodString(String terminalStr ) {
    ...
    methodTypes types = this.decodeMethodDesc( descriptor.getString() );
    ...
    String cmntStr = this.commentString();
    // get any exceptions
    String excptStr = this.exceptionString();
  ...
  void pr() {
	...
	if (len > 0) {
	  System.out.println(indent1 + this.methodString( " {" ) );
	  printed_header = true;
	}
...
class classAttrSec extends dataRead {
  ...
  classAttrSec( DataInputStream dStream, constPool constPoolSec ) {
    ...
    int numAttr = this.readU2( dStream );
