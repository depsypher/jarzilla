...
public class codeAttr extends attrInfo {
  ...
  class exceptInfo {
    ...
    exceptInfo( DataInputStream dStream, constPool constPoolSec ) {
      startPC = codeAttr.this.readU2( dStream );
      endPC = codeAttr.this.readU2( dStream );
      handlerPC = codeAttr.this.readU2( dStream );
      int catchTypeIx = codeAttr.this.readU2( dStream );
  ...
  public codeAttr( String name, int length,
    ...
    max_stack = this.readU2( dStream );
    max_locals = this.readU2( dStream );
    int codeLen = this.readU4( dStream );
	...
	codeBuf[i] = (byte)this.readU1( dStream );
      } // for
    }

    int exceptLen = this.readU2( dStream );
    ...
    int attrCount = this.readU2( dStream );
